...
import { LocalStorageProvider, SettingsProvider } from '@apicize/engine'
import MenuBuilder from './menu'
import { ApicizeEvents, Workbook, WorkbookAuthorization, WorkbookTest } from '@apicize/definitions'
...
const initHandlersAndWindow = (): void => {
  ...
  ipcMain.handle(ApicizeEvents.SaveWorkbookToFile, async (_: IpcMainInvokeEvent, workbook: Workbook, ...name: string[]) => {
    const result = await localStorage.saveWorkbook(workbook, ...name)
    if (! (result instanceof Error)) {
      mainWindow.setTitle(`Apicize - ${result.displayName}`)
    }
    return result
  ...
  // ipcMain.handle(ApicizeEvents.RunTests, async (_: IpcMainInvokeEvent, tests: WorkbookTest[], authorization: WorkbookAuthorization) => {
